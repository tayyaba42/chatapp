...
void inserttodb(int client_id,std::string receivedMessage,std::string timestamp){
...
int main() {
    ...
    //we can reuse any port which is already inuse
    int reuse = 1;
    ...
    cout<<"--Server listening to port: "<<port<<": waiting for connections--"<<endl;
    std::time_t now = std::time(nullptr);
    std::string uptime = std::ctime(&now);
    cout << "Server uptime: " << uptime;
    cout<<endl;
    ...
    createdb();
    threads[i] = thread(client,client_socket,client_id);
    cout << "--Client:"<<clientCounter<<" connected successfully--"<<endl<<endl;
